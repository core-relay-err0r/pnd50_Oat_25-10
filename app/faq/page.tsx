          ...
          <div className="max-w-4xl">
            ...
            <div className="relative max-w-2xl">
              ...
              <Input
                ...
                className="pl-12 pr-4 py-6 text-base bg-white/10 backdrop-blur-sm border-white/20 text-white placeholder:text-slate-400 focus:bg-white/15 focus:border-primary/50 transition-all"
              />
            </div>
          </div>
        </div>
            ...
            {faqCategories.map((category) => {
              ...
              return (
                <a
                  key={category.id}
                  href={`#${category.id}`}
                  className="group flex items-center gap-2 px-4 py-2 rounded-full bg-card border border-border hover:border-primary/50 hover:bg-primary/5 transition-all"
                >
      ...
      <div className="container mx-auto px-4 py-20">
        <div className="max-w-5xl mx-auto space-y-16">
                ...
                <section key={category.id} id={category.id} className="scroll-mt-24">
                  <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-14 h-14 rounded-2xl flex items-center justify-center border ${colorClasses[category.color as keyof typeof colorClasses]}`}
                      >
                  ...
                  <Accordion type="single" collapsible className="space-y-4">
                    {category.questions.map((q) => (
                      <AccordionItem
                        key={q.id}
                        value={q.id}
                        className="group bg-card border border-border rounded-2xl px-6 md:px-8 data-[state=open]:shadow-xl data-[state=open]:border-primary/30 transition-all duration-300 hover:shadow-lg hover:border-border/80"
                      >
                        <AccordionTrigger className="text-left hover:no-underline py-6">
                          <div className="flex items-start gap-3 pr-4">
                            <ChevronRight className="w-5 h-5 text-primary mt-1 flex-shrink-0 group-data-[state=open]:rotate-90 transition-transform" />
                            <span className="font-semibold text-foreground text-base md:text-lg leading-relaxed">
                              {q.question}
                            </span>
                          </div>
                        </AccordionTrigger>
                        <AccordionContent className="text-muted-foreground leading-relaxed space-y-4 pb-6 pl-8">
                          {q.answer}
                        </AccordionContent>
                      </AccordionItem>
                    ))}
          ...
          <section className="mt-20">
            <div className="relative overflow-hidden bg-gradient-to-br from-primary/10 via-primary/5 to-background border-2 border-primary/20 rounded-3xl p-8 md:p-12">
              {/* Decorative elements */}
