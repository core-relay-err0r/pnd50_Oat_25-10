...
import {
  ...
  type RegisteredOfficeOptions,
} from "./RegisteredOfficePopup";
import {
  CompanyReviewPopup,
  type CompanyReviewOptions,
} from "./CompanyReviewPopup";
import {
  CorporateDocumentsPopup,
  type CorporateDocumentsOptions,
} from "./CorporateDocumentsPopup";
import {
  TaxResidencyPopup,
  type TaxResidencyOptions,
} from "./TaxResidencyPopup";
...
export function CorporateServices() {
  ...
  const isSelected = (id: string) =>
    selectedCorporateServices.some((s) => s.id === id);
  ...
  const handleCorporateDocumentsConfirm = (
    options: CorporateDocumentsOptions
  ) => {
  ...
  const totalPrice = selectedCorporateServices.reduce(
    (sum, s) => sum + s.price,
    0
  );
  ...
  return (
    <div className="space-y-12">
      ...
      <p className="text-center text-xs text-muted-foreground/60">
        Advanced structures (e.g. BOI) usually make sense only after operations
        begin.
      </p>
